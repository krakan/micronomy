...
sub get-month(:$token is copy, :$date) is export {
    ...
    get-period(:$token, :$start-date, :$end-date);

    CATCH {
        default {
            error $_, $token;
            return get-login(reason => "okänt fel");
        }
    }
}
...
sub get-period(Str :$token is copy, Date :$start-date, Date :$end-date, Int :$hours-cache is copy = 0) is export {
    ...
    CATCH {
        when X::Cro::HTTP::Error {
            error $_, $token;
            return get-login(reason => "Vänligen logga in!") if .response.status == 401;
            return get-login(reason => "felstatus {.response.status}");
        }
        default {
            error $_, $token;
            return get-login(reason => "okänt fel");
...
sub get(:$token is copy, :$date = '') is export {
    $token = fix-token($token);
...
sub edit(:$token is copy, :%parameters) is export {
    $token = fix-token($token);
    trace "edit", $token;
...
sub set(:$token is copy, :%parameters) is export {
    $token = fix-token($token);
    trace "set", $token;
    ...
    return get-login(reason => "Vänligen logga in!") unless $token;
    ...
    CATCH {
        when X::Cro::HTTP::Error {
            if .response.status == 401 {
                return get-login(reason => "Vänligen logga in!");
            } else {
                error $_, $token;
                %parameters<concurrency>:delete;
                %content = get-week($token, %parameters<date>, previous => %parameters) unless %content;
...
sub submit(:%parameters, :$token is copy) is export {
    $token = fix-token($token);
    ...
    CATCH {
        when X::Cro::HTTP::Error {
            ...
            error $_, $token;
            if .response.status == 401 {
                get-login(reason => "Vänligen logga in!");
            } else {
...
sub get-login(:$username = '', :$reason = '') is export {
                ...
                CATCH {
                    when X::Cro::HTTP::Error {
                        ...
                        if $status eq "[401] An internal error occurred." and $wait < 9 {
                            ...
                            return get-login(reason => "fel användarnamn eller lösenord");
                        } elsif .response.status == 500 {
                            trace "login failed '$status' - restarting", $token;
                            get-login(reason => "$status - försök igen om ett tag");
                            sleep 2;
                            exit 1;
                        } else {
                            return get-login(reason => "$status");
                        }
                    }
                    default {
                        error $_, $token;
                        trace "login failed - restarting", $token;
                        get-login(reason => "okänt fel - försök igen om ett tag");
                        sleep 2;
                        exit 1;
                    }
                }
...
sub logout(:$token is copy) is export {
    $token = fix-token($token);
    trace "logout", $token;
    my $status;
...
sub calendar(:$date,:$token is copy,) is export {
